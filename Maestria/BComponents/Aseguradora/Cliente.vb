...
#Region " Search Methods "
        ...
        Public Function Search(ByVal Id As Int32, ByVal ParamArray Relations() As [Enum]) As MEB.Cliente
            Dim BEObject As MEB.Cliente = Nothing

            Try
                Using DALObject As New DAL.Cliente
...
#Region " List Methods "
        ...
        Public Function List(ByVal UnidadNegocioId As Int32, ByVal CantidadRegistros As Int32, ByVal NumeroPagina As Int32, ByVal ParamArray Relations() As [Enum]) As List(Of MEB.Cliente)
            Try
                Dim BECollection As List(Of MEB.Cliente)
    ...
    End Function
        ...
        Public Function ListadoUN(ByVal UnidadNegocioId As Int32, ByVal ParamArray Relations() As [Enum]) As List(Of MEB.Cliente)
            Try
                Dim BECollection As List(Of MEB.Cliente)
                Using DALObject As New DAL.Cliente
                    BECollection = DALObject.ListadoUN(UnidadNegocioId, Relations)
                End Using
                Return BECollection
            Catch ex As Exception
                MyBase.ErrorHandler(ex, ErrorPolicy.BCWrap)
        ...
        ''' <remarks>
        ''' </remarks>
        Public Function List(ByVal IdMaster As Long, ByVal ParamArray Relations() As [Enum]) As List(Of MEB.Cliente)
            Try
                Dim BECollection As List(Of MEB.Cliente)
...
#Region " Save Methods "
        ...
        ''' <remarks>
        ''' </remarks>
        Public Sub Save(ByRef BEObj As MEB.Cliente)
            Dim DALObject As DAL.Cliente = Nothing
        ...
        ''' <remarks>
        ''' </remarks>
        Friend Sub Save(ByRef colBEObj As List(Of MEB.Cliente), ByVal IdMaster As Long)
            Dim DALObject As DAL.Cliente = Nothing
            Try
                DALObject = New DAL.Cliente
                DALObject.OpenConnection()
        ...
        ''' <remarks>
        ''' </remarks>
        Friend Function Validate(ByRef BEObj As MEB.Cliente) As Boolean
