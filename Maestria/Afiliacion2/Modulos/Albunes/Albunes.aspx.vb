...
Public Class Albunes
    ...
    Private Sub CrearGaleria2(listaAdjuntos As List(Of BEB.Foto))
        ...
        'Ocultito.Value = ""
        'Dim StringUrls As String = ""
        'Dim contador As Boolean = False


        'sb.Append("var $lg = $('#divDetail3');")
        'sb.AppendLine()
        'sb.Append("console.log($lg);")
        'sb.AppendLine()
        'sb.Append("$lg.lightGallery();")
        'sb.AppendLine()
        'sb.Append("$lg.on('onCloseAfter.lg',function(event){")
        'sb.AppendLine()
        'sb.Append("alert('Destruyendome');")
        'sb.AppendLine()
        'sb.Append("$lg.data('lightGallery').destroy(true);")
        'sb.AppendLine()
        'sb.Append("});")
        'sb.AppendLine()
        ...
        'sb.AppendLine()
        'sb.Append("console.log(gallery);")
        'sb.AppendLine()
        'sb.Append("gallery.removeData(); // destroy gallery")
        'sb.AppendLine()
        'sb.Append("gallery = $('#divDetail3').lightGallery();")
        'sb.AppendLine()
        'sb.Append("}, 100);")
        'sb.AppendLine()
...
if($('#divDetail3').data('lightGallery') === undefined){}else{
    ...
    Private Sub proInit_Form()
        Try
            ...
            Dim ListAlbum = bcAlbum.ListBuscar(rtbFilter.Text.Trim, rgvGrid.MasterTableView.CurrentPageIndex, rgvGrid.MasterTableView.PageSize)
    ...
    'Private Sub rbAdjuntar_Click(sender As Object, e As EventArgs) Handles rbAdjuntar.Click
    '    If (CargadorArchivo.UploadedFiles IsNot Nothing) Then
    '        Dim imagenes As System.IO.MemoryStream = New System.IO.MemoryStream
    '        imagenes = CargadorArchivo.UploadedFiles(0).InputStream
    '        Dim archivo As Byte() = imagenes.ToArray
    '        Dim BEHotelesAdjuntos As BEB.HotelAdjunto = New BEB.HotelAdjunto
    '        With BEHotelesAdjuntos
    '            .Descripcion = CargadorArchivo.UploadedFiles(0).FileName
    '            .Url = CargadorArchivo.UploadedFiles(0).FileName
    '            .HotelId = 1
    '            .Archivo = archivo
    '            .Estado = BEntities.Estado.Activo
    '            .FechaReg = Now
    ...
    '            .PersonalId = funGet_UserCode()
    '        End With
    '        listaHotelesAdjuntos.Add(BEHotelesAdjuntos)
    ...
    '    If Not Page.IsPostBack Then
    '        rbAdjuntar.Attributes.Add("onclick", String.Format("realPostBack('{0}', ''); return false;", rbAdjuntar.UniqueID))
    '        'proInit_Form()
    '        Session("archivo") = 0
    '    Else
    '        CargadorArchivo.SaveAs("C:\Uploads\imagenhotel_" & Now.ToString("yyyyMMddHHmm") & Path.GetExtension(CargadorArchivo.FileName))
    '        Session("archivo") = "C:\Uploads\imagenhotel_" & Now.ToString("yyyyMMddHHmm") & Path.GetExtension(CargadorArchivo.FileName)
    '    End If
    'End Sub
    'Public Sub rbAdjuntar_Click(sender As Object, e As EventArgs) Handles rbAdjuntar.Click
    'Try
    '    If (aqui.HasFile) Then
    '        Dim holapas = aqui.FileName
    '        Dim chau = ""
    '    End If
    '    If CargadorArchivo.HasFile Then
    '        If Path.GetExtension(CargadorArchivo.FileName).Equals(".jpg") Or Path.GetExtension(CargadorArchivo.FileName).Equals(".png") Then
    '            Dim imagenes As Byte()
    '            imagenes = CargadorArchivo.FileBytes
    '            'Dim archivo As Byte() = imagenes.ToArray
    '            Dim BEHotelesAdjuntos As BEB.HotelAdjunto = New BEB.HotelAdjunto
    '            With BEHotelesAdjuntos
    '                .StatusType = BEntities.StatusType.Insert
    '                .Descripcion = CargadorArchivo.FileName
    '                .Url = CargadorArchivo.FileName
    '                .HotelId = 1
    '                .Archivo = imagenes
    '                .Estado = BEntities.Estado.Activo
    '                .FechaReg = Now
    ...
    '        Else
    '            MyBase.proShow_Message("La importacion no permite el tipo de extension *" & Path.GetExtension(CargadorArchivo.FileName))
    '        End If
    '    End If
    'Catch ex As Exception
    ...
    '    Dim folderPath As String = Server.MapPath("~/assets/")
