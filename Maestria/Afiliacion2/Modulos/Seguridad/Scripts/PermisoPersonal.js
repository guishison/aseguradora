...
function funValidate(sender, args) {
    ...
    var rbManual = $find(funGet_ClientID("rbManual"));
    var rbPersonal = $find(funGet_ClientID("rbPersonal"));
    var rbPlantilla = $find(funGet_ClientID("rbPlantilla"));
    var rcbPersonal = $telerik.findComboBox(AVA.getID("rcbPersonal"));
    var rcbPlantilla = $telerik.findComboBox(AVA.getID("rcbPlantilla"));
    //if (rbManual.get_checked() == false && rbPersonal.get_checked() == false && rbPlantilla.get_checked() == false) {
    //        strMessage += " - Seleccione Una Opcion 'Manual, Personal o por Plantillas' <br />";
    //}
    if (rbPersonal.get_checked() == true) {
        if (rcbPersonal.get_value() == "-1") {
            strMessage += " - Seleccione un personal <br />";
        }
    }
    if (rbPlantilla.get_checked() == true) {
        if (rcbPlantilla.get_value() == "-1") {
            strMessage += " - Seleccione una plantilla <br />";
        }
    }
...
function ConfirmDelete(sender, args) {
    ...
    var text = "Esta seguro de eliminar el personal seleccionado?";
