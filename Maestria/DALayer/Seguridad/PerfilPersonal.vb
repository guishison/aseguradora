...
#Region " Save Methods"
        ...
        ''' <remarks>
        ''' </remarks>
        Public Sub Guardar(ByRef BEObj As SEC.PerfilPersonal)
            Dim strQuery As String = ""
            If BEObj.StatusType = BE.StatusType.Insert Then
                strQuery = "crm_seguridad_perfilpersonal_insert"
            ElseIf BEObj.StatusType = BE.StatusType.Update Then
                strQuery = "UPDATE security_msprofile SET " &
                    "UserId = @UserId,Profile = @Profile,Hidden = @Hidden,registry = @registry " &
                    "WHERE Id = @Id"
            ElseIf BEObj.StatusType = BE.StatusType.Delete Then
                strQuery = "DELETE FROM security_msprofile WHERE Id = @Id"
            End If
            ...
            Try
                ...
                If BEObj.StatusType <> BE.StatusType.NoAction Then
                    ...
                    MyBase.DBFactory.AddInParameter(MyBase.Command, "@PerfilId", DbType.Int32, BEObj.PerfilId)
...
#Region " Methods "
        ...
        ''' <remarks>
        ''' </remarks>
        Friend Function ReturnMaster(ByVal Id As Long, ByVal ParamArray Relaciones() As [Enum]) As SEC.PerfilPersonal
        ...
        ''' <remarks>
        ''' </remarks> 
        Friend Function ReturnChild(ByVal Keys As IEnumerable(Of Int32), ByVal ParamArray Relaciones() As [Enum]) As List(Of SEC.PerfilPersonal)
            Dim llaves As String = MyBase.KeysArray(Keys).ToString.Replace("(", "(',")
            llaves = llaves.Replace(")", ",')")
            Dim strQuery As String = "crm_seguridad_perfilpersonal_buscarpersonales"
            Try
                MyBase.Command = MyBase.DBFactory.GetStoredProcCommand(strQuery)
                MyBase.DBFactory.AddInParameter(MyBase.Command, "@llaves", DbType.String, llaves)
                Dim Coleccion As List(Of SEC.PerfilPersonal) = MyBase.SQLConvertidorIDataReaderListas(Of SEC.PerfilPersonal)(MyBase.DBFactory.ExecuteReader(MyBase.Command))
        ...
        Friend Function ReturnChild3(ByVal Keys As IEnumerable(Of Int32), ByVal ParamArray Relaciones() As [Enum]) As List(Of SEC.PerfilPersonal)
            ...
            Dim Coleccion As List(Of SEC.PerfilPersonal) = Nothing
            Coleccion = MyBase.SQLList(Of SEC.PerfilPersonal)(strQuery)
            If Coleccion.Count > 0 Then
                Me.CargarRelaciones(Coleccion, Relaciones)
            End If
            Return Coleccion
        ...
        Friend Function ReturnChildById(ByVal Keys As IEnumerable(Of Int32), ByVal ParamArray Relaciones() As [Enum]) As List(Of SEC.PerfilPersonal)
        ...
        Friend Function ReturnChild2(ByVal Keys As IEnumerable(Of Int32), ByVal ParamArray Relaciones() As [Enum]) As SEC.PerfilPersonal
            Dim strQuery As String = "SELECT * FROM security_msprofile WHERE Id IN " & MyBase.KeysArray(Keys)
            Dim BEProfile As SEC.PerfilPersonal = Nothing
            BEProfile = MyBase.SQLSearch(Of SEC.PerfilPersonal)(strQuery)
        ...
        Protected Overrides Function LoadBE(Of T As BE.BEntity)(ByRef DR As IDataReader) As T
            Dim BEObject As New SEC.PerfilPersonal

            With DR
                BEObject.Id = .GetInt32(0)
                BEObject.PersonalId = .GetInt32(1)
                BEObject.PerfilId = .GetInt32(2)
                BEObject.UnidadNegocioId = .GetInt32(3)
                BEObject.SupervisorId = .GetInt32(4)
                BEObject.FechaReg = .GetDateTime(5)
                BEObject.FechaActualizacion = .GetDateTime(6)
                BEObject.PersonalIdL = .GetInt32(7)
        ...
        ''' <remarks></remarks>
        Protected Sub CargarRelaciones(ByRef Coleccion As List(Of SEC.PerfilPersonal), ByVal ParamArray Relaciones() As [Enum])
            ...
            For Each RelationEnum As [Enum] In Relaciones
                If RelationEnum.Equals(SEC.relPerfilPersonal.Personal) Then
                ...
                Keys = From BEPerfil In Coleccion Select BEPerfil.Id
                If RelationEnum.Equals(SEC.relPerfilPersonal.Formulario) Then
                ...
                End If
                Keys = From BEPerfilPersonal In Coleccion Select BEPerfilPersonal.PerfilId
                If RelationEnum.Equals(SEC.relPerfilPersonal.Perfil) Then
                    DALPerfil = New Perfil(True, CType(MyBase.DBFactory, Object))
                    colPerfil = DALPerfil.ReturnChildPerfilId(Keys, Relaciones)
                End If
            ...
            If Relaciones.GetLength(0) > 0 Then
                For Each BEProfile In Coleccion
                    ...
                    If colPerfil IsNot Nothing Then
                        BEProfile.Perfil = (From BEObject In colPerfil
                                            Where BEObject.Id = BEProfile.PerfilId
                                            Select BEObject).FirstOrDefault
                    End If
        ...
        ''' <param name="Relaciones">Relacioneship enumerator</param>
        ''' <remarks></remarks>
        Protected Sub CargarRelaciones(ByRef BEProfile As SEC.PerfilPersonal, ByVal ParamArray Relaciones() As [Enum])
            Dim DALFormulario As Formulario
            Dim DALPersonal As Personal
...
#Region " List Methods "
        ...
        ''' <remarks>
        ''' </remarks>
        Public Function Listar(ByVal Order As String, ByVal ParamArray Relaciones() As [Enum]) As List(Of SEC.PerfilPersonal)
            Dim strQuery As String = "SELECT * FROM security_msprofile ORDER By " & Order
            Dim Coleccion As List(Of SEC.PerfilPersonal) = MyBase.SQLList(Of SEC.PerfilPersonal)(strQuery)
        ...
        Public Function PagedFilterList(PageIndex As Integer, PageSize As Integer, OrderBy As String, ByVal ParamArray Relaciones() As [Enum]) As List(Of SEC.PerfilPersonal)
            Dim strQuery As String = "SELECT * FROM security_msprofile Order By " & OrderBy
            Dim Coleccion As List(Of SEC.PerfilPersonal) = MyBase.PaggingSQLList(Of SEC.PerfilPersonal)(strQuery, PageIndex, PageSize)
        ...
        ''' <remarks>
        ''' </remarks>
        Public Function Listar(ByVal IdPadre As Long, ByVal ParamArray Relaciones() As [Enum]) As List(Of SEC.PerfilPersonal)
        ...
        Friend Function ReturnOption(ByVal Keys As IEnumerable(Of Int32), ByVal ParamArray Relaciones() As [Enum]) As List(Of BE.Relation)
            Dim strQuery As String = "SELECT IT.* FROM security_psprofileoption IT INNER JOIN " &
                                         "security_msprofile A ON A.Id = IT.PerfilPersonalId WHERE IT.PerfilPersonalId in " & MyBase.KeysArray(Keys)
...
#Region " Search Methods "
        ...
        ''' <remarks>
        ''' </remarks>
        Public Function Buscar(ByVal Id As Long, ByVal ParamArray Relaciones() As [Enum]) As SEC.PerfilPersonal
            Dim strQuery As String = "SELECT * FROM security_msprofile WHERE Id = " & Id.ToString
            Dim BEProfile As SEC.PerfilPersonal = MyBase.SQLSearch(Of SEC.PerfilPersonal)(strQuery)
