...
#Region " Save Methods"
        ...
        Public Sub Guardar(ByRef BEObj As SEC.Perfil)
            ...
            If BEObj.StatusType = BE.StatusType.Insert Then
                strQuery = "INSERT INTO security_msprofile " &
                    "VALUES(@Id,@UserId,@Profile,@Hidden,@registry); SELECT @@IDENTITY"
...
#Region " Methods "
        ...
        ''' <remarks>
        ''' </remarks>
        Friend Function ReturnMaster(ByVal Id As Long, ByVal ParamArray Relaciones() As [Enum]) As SEC.Perfil
        ...
        Friend Function ReturnChild(ByVal Keys As IEnumerable(Of Int32), ByVal ParamArray Relaciones() As [Enum]) As List(Of SEC.Perfil)
            Dim strQuery As String = "SELECT * FROM security_msprofile WHERE UserId IN " & MyBase.KeysArray(Keys)
            Dim Coleccion As List(Of SEC.Perfil) = Nothing
            Coleccion = MyBase.SQLList(Of SEC.Perfil)(strQuery)
        ...
        Friend Function ReturnChildPerfilId(ByVal Keys As IEnumerable(Of Int32), ByVal ParamArray Relaciones() As [Enum]) As List(Of SEC.Perfil)
            ...
            Try
                ...
                Dim Coleccion As List(Of SEC.Perfil) = MyBase.SQLConvertidorIDataReaderListas(Of SEC.Perfil)(MyBase.DBFactory.ExecuteReader(MyBase.Command))
                If Coleccion.Count > 0 Then
                    Me.CargarRelaciones(Coleccion, Relaciones)
                End If
                Return Coleccion
        ...
        Friend Function ReturnChild3(ByVal Keys As IEnumerable(Of Int32), ByVal ParamArray Relaciones() As [Enum]) As List(Of SEC.Perfil)
        ...
        Friend Function ReturnChildById(ByVal Keys As IEnumerable(Of Int32), ByVal ParamArray Relaciones() As [Enum]) As List(Of SEC.Perfil)
        ...
        Friend Function ReturnChild2(ByVal Keys As IEnumerable(Of Int32), ByVal ParamArray Relaciones() As [Enum]) As SEC.Perfil
            Dim strQuery As String = "SELECT * FROM security_msprofile WHERE Id IN " & MyBase.KeysArray(Keys)
            Dim BEProfile As SEC.Perfil = Nothing
            BEProfile = MyBase.SQLSearch(Of SEC.Perfil)(strQuery)
        ...
        Protected Overrides Function LoadBE(Of T As BE.BEntity)(ByRef DR As IDataReader) As T
            Dim BEObject As New SEC.Perfil
        ...
        ''' <remarks></remarks>
        Protected Sub CargarRelaciones(ByRef Coleccion As List(Of SEC.Perfil), ByVal ParamArray Relaciones() As [Enum])
        ...
        ''' <param name="Relaciones">Relacioneship enumerator</param>
        ''' <remarks></remarks>
        Protected Sub CargarRelaciones(ByRef BEProfile As SEC.Perfil, ByVal ParamArray Relaciones() As [Enum])
            Dim DALFormulario As Formulario
            Dim DALPersonal As Personal
...
#Region " List Methods "
        ...
        ''' <remarks>
        ''' </remarks>
        Public Function Listar(ByVal Order As String, ByVal ParamArray Relaciones() As [Enum]) As List(Of SEC.Perfil)
            Dim strQuery As String = "SELECT * FROM security_msprofile ORDER By " & Order
            Dim Coleccion As List(Of SEC.Perfil) = MyBase.SQLList(Of SEC.Perfil)(strQuery)
        ...
        Public Function Listar(ByVal ParamArray Relaciones() As [Enum]) As List(Of SEC.Perfil)
            Dim strQuery As String = "crm_seguridad_perfil_lista"
            Try
                MyBase.Command = MyBase.DBFactory.GetStoredProcCommand(strQuery)
                Dim Collection As List(Of SEC.Perfil) = MyBase.SQLConvertidorIDataReaderListas(Of SEC.Perfil)(MyBase.DBFactory.ExecuteReader(MyBase.Command))
                Return Collection
            Catch ex As Exception
                MyBase.ErrorHandler(ex, ErrorPolicy.DALWrap)
                Return Nothing
        ...
        Public Function PagedFilterList(PageIndex As Integer, PageSize As Integer, OrderBy As String, ByVal ParamArray Relaciones() As [Enum]) As List(Of SEC.Perfil)
            Dim strQuery As String = "SELECT * FROM security_msprofile Order By " & OrderBy
            Dim Coleccion As List(Of SEC.Perfil) = MyBase.PaggingSQLList(Of SEC.Perfil)(strQuery, PageIndex, PageSize)
        ...
        ''' <remarks>
        ''' </remarks>
        Public Function Listar(ByVal IdPadre As Long, ByVal ParamArray Relaciones() As [Enum]) As List(Of SEC.Perfil)
        ...
        Friend Function ReturnOption(ByVal Keys As IEnumerable(Of Int32), ByVal ParamArray Relaciones() As [Enum]) As List(Of BE.Relation)
            Dim strQuery As String = "SELECT IT.* FROM security_psprofileoption IT INNER JOIN " &
                                         "security_msprofile A ON A.Id = IT.PerfilId WHERE IT.PerfilId in " & MyBase.KeysArray(Keys)
...
#Region " Search Methods "
        ...
        ''' <remarks>
        ''' </remarks>
        Public Function Buscar(ByVal Id As Long, ByVal ParamArray Relaciones() As [Enum]) As SEC.Perfil
            Dim strQuery As String = "SELECT * FROM security_msprofile WHERE Id = " & Id.ToString
            Dim BEProfile As SEC.Perfil = MyBase.SQLSearch(Of SEC.Perfil)(strQuery)
